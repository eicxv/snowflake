...
vec3 vaporColor2(vec4 cell) {
    ...
    // vec3 col1 = vec3(0);
    // vec3 col2 = vec3(0.965, 0.878, 0.647);
...
vec3 iceColor2(vec4 cell) {
    ...
    // float c = cell.z / u_rho;
    // c *= 0.4;
    // c += 0.6;
    // vec3 col1 = vec3(0.022, 0.048, 0.16);
    // vec3 col2 = vec3(0.700, 0.742, 0.88);
...
void main () {
    ...
    if (cell.x < 0.5 && cell.w == 0.0) {
        cell.w = u_rho;
    }

    bool isFrozen = cell.x > 0.5;
    // vec3 c = isFrozen ? iceColor2(cell) : vaporColor2(cell);
